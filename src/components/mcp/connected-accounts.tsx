...
import { deleteConnectedAccount } from "@/app/(chat)/mcp/accounts/actions";
...
export function ConnectedAccounts({ accounts }: ConnectedAccountsProps) {
  ...
  if (!accounts || accounts.length === 0) {
    return (
      <div className="rounded-md border p-4 bg-background max-w-3xl">
        ...
        <p className="py-4">
          Your connected accounts will be listed here after you connect them,
  ...
  const sortedAccounts = [...accounts].sort((a, b) => {
    ...
    const appNameComparison = (a.app.name || "").localeCompare(
      b.app.name || "",
    );
    ...
    <>
      ...
      <div className="grid gap-3 max-w-3xl">
        {sortedAccounts.map((account) => (
          <div key={account.id} className="flex flex-col p-3 rounded-md border">
