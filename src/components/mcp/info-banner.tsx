...
function Banner({
  ...
  return (
    <div
      className={`flex items-start gap-2 sm:gap-3 p-2 sm:px-3 rounded-md border text-sm font-medium ${containerClasses}`}
    >
...
export function InfoBanner({
  isAuthDisabled: isAuthDisabledMode,
  ...
  const showPersistenceWarning =
    isPersistenceDisabled && !isBannerDismissed("persistence");
    ...
    <div className={`flex flex-col gap-2 ${className}`}>
      {showMainBanner && (
        ...
        >
          {isAuthDisabledMode ? (
            <span>
              User sign-in is currently disabled, make sure to enable before
              shipping to production.
            </span>
          ) : (
            <span>
              Welcome to your AI Assistant! Connect your favorite apps and tools
      ...
      {showPersistenceWarning && (
        <Banner theme="warning" onDismiss={() => dismissBanner("persistence")}>
          <span>
            Chat storage is currently disabled. Your conversations will not be
            saved.
          </span>
        </Banner>
      )}
