...
import { Hammer, MicIcon, MicOffIcon, LoaderIcon } from "lucide-react";
...
function PureMultimodalInput({
  ...
  const { data: _session, status: authStatus } = useEffectiveSession();
  ...
  // Auto-insert transcript into input when available
  useEffect(() => {
    if (transcript) {
      ...
      // Focus textarea after inserting transcript
      setTimeout(() => {
        textareaRef.current?.focus();
      }, 0);
    }
  }, [transcript, input, setInput, clearTranscript]);
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      const response = await fetch("/api/mcp-chat/files/upload", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="relative w-full flex flex-col gap-4">
      ...
      <div className="absolute bottom-0 p-2 w-fit flex flex-row justify-start space-x-1">
        ...
        /> */}
        <AppsButton status={status} setAppSelectorOpen={setAppSelectorOpen} />
        <MicButton
          status={status}
...
function PureMicButton({
  ...
  const handleMicClick = async (event: React.MouseEvent) => {
    ...
    if (isRecording) {
      ...
      if (!checkCreditsForAction("voice", 1)) {
        return; // Don't start recording if no credits
      }
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            disabled={
              status !== "ready" ||
              isTranscribing ||
              (!hasCredits && !isRecording)
            }
            variant="ghost"
            aria-label={
              isRecording ? t("stopRecording") : t("startVoiceRecording")
            }
          >
            {isTranscribing ? (
        ...
        <TooltipContent>
          <p>
            {!hasCredits && !isRecording
              ? "No credits for voice transcription"
              : isTranscribing
                ? t("transcribing")
                : isRecording
                  ? t("stopRecording")
