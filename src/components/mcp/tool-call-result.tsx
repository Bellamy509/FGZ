...
import { prettifyToolName } from "@/lib/mcp-chat/utils";
...
export const ToolCallResult = ({
  ...
  const connectLinkRegex =
    /https:\/\/pipedream\.com\/_static\/connect\.html[^\s]*/;
        ...
        {name === "Web_Search" ? (
          ...
          appId && (
            <div className="flex items-center justify-center size-5 bg-white dark:bg-gray-100 rounded-sm overflow-hidden mr-1">
              <img src={iconUrl} alt="icon" className="size-4 rounded" />
            </div>
          )
        )}
            ...
            >
              {appId && (
                <div className="flex items-center justify-center size-6 bg-white dark:bg-gray-100 rounded-sm overflow-hidden mr-1">
                  <img src={iconUrl} alt="icon" className="size-5 rounded" />
                </div>
              )}
              Connect account
            </Button>
