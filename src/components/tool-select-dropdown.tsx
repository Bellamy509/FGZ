...
// Tools to hide from the tool setup UI
const HIDDEN_TOOL_NAMES = ["code", "Code"];
      ...
      .map((server) => {
        ...
        return {
          ...
          tools: server.toolInfo
            .filter((tool) => !HIDDEN_TOOL_NAMES.includes(tool.name))
            .map((tool) => ({
              name: tool.name,
...
function AppDefaultToolKitSelector() {
  ...
  const defaultToolInfo = useMemo(() => {
    ...
    return Object.values(AppDefaultToolkit)
      .filter((toolkit) => !HIDDEN_TOOL_NAMES.includes(toolkit))
      .map((toolkit) => {
