...
import { pdHeaders } from "../pd-backend-client";
...
It is also used to validate the input of the language model.
...
class MCPSessionManager {
  ...
  /**
   * Shortens tool names to fit OpenAI's 64-character limit
   */
  private shortenToolName(name: string): string {
    if (name.length <= 64) {
      return name;
    }

    // Try removing prefixes and suffixes first
    let shortened = name
      .replace(/^BEGIN_CONFIGURATION_/, "CONFIG_")
      .replace(/_CALENDAR/, "_CAL")
      .replace(/_OUTLOOK/, "_OL")
      .replace(/_MICROSOFT/, "_MS")
      .replace(/_CONFIGURATION/, "_CFG")
      .replace(/_EVENT/, "_EVT")
      .replace(/_DELETE/, "_DEL")
      .replace(/_CREATE/, "_NEW")
      .replace(/_UPDATE/, "_UPD")
      .replace(/_ATTENDEES/, "_ATT");

    // If still too long, truncate and add hash for uniqueness
    if (shortened.length > 64) {
      const hash = Math.abs(
        ...
        .toString(16)
        .slice(0, 6);
      shortened = shortened.slice(0, 57) + "_" + hash;
    }
  ...
  private convertTools(
    ...
    for (const mcpTool of Object.values(mcpTools)) {
      const shortenedName = this.shortenToolName(mcpTool.name);
      tools[shortenedName] = tool({
        description: mcpTool.description || "",
        parameters: jsonSchema(mcpTool.inputSchema as any),
        execute: async (args: unknown, options: ToolExecutionOptions) => {
          return this.executeTool(mcpTool.name, args as any, {
